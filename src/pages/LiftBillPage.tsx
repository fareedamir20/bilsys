...
export function LiftBillPage({ user }: { user: User | null }) {
  ...
  const validCategories = settings.liftCategories.filter(c => {
  ...
  const calculateTotal = () => {
    ...
    additional.forEach(a => {
      if (a.title.trim() !== '' && a.amount > 0) sum += a.amount;
    });
    return sum;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Max bills check
    const userBillsThisMonthType = allBills.filter(b => b.userId === user.id && b.month === month && b.year === year && b.type === 'lift');
    if (userBillsThisMonthType.length >= settings.maxBillsPerMonth) {
        return toast.error(`You have reached the maximum number of Lift bills (${settings.maxBillsPerMonth}) for ${month} ${year}.`);
    }
    ...
    try {
      ...
      await pushActivityLog({
        ...
        details: `Generated LESCO Lift Bill for ${month} ${year} (Total: Rs ${totalAmt})`,
        timestamp: new Date().toISOString()
      });

      generateBillPDF(newBill);
      toast.success('LESCO Lift Bill generated and PDF downloaded successfully.');
      ...
      console.error(e);
      toast.error('Failed to generate Lift bill');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl mx-auto space-y-8 pb-32">
      <div>
        <h1 className="text-3xl font-heading font-bold mb-2">Generate LESCO Lift Bill</h1>
        <p className="text-muted-foreground">Create a new lift maintenance and electricity bill. The PDF will be automatically generated.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="glass-card p-6 space-y-6">
            <h3 className="font-heading font-semibold text-lg border-b border-border/50 pb-2">Lift Expense Details</h3>
                ...
                <div key={category.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  ...
                  <div className="relative w-full sm:w-48">
                    ...
                    <input 
                      type="number" 
                      min="0"
                      className="input-field w-full pl-9 bg-emerald-500/5 focus:ring-emerald-500" 
            ...
            <div className="pt-6 border-t border-border/50 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div>
                ...
                <p className="text-3xl font-heading font-bold text-emerald-500">Rs {calculateTotal().toLocaleString()}</p>
              </div>
              
              <button 
                type="submit" 
                disabled={submitting}
                className="btn-primary flex items-center justify-center gap-2 w-full sm:w-auto bg-emerald-600 hover:bg-emerald-700 text-white"
              >
                {submitting ? <Loader2 className="w-5 h-5 animate-spin" /> : <FileCheck2 className="w-5 h-5" />}
                {submitting ? 'Generating...' : 'Generate Lift Bill'}
              </button>
            </div>
